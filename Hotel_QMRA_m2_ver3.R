...
run_touch_sequence_2 <- function(sequence, scenario, iter, intervention = NULL) {
  ...
  for (i in seq_along(sequence)) {
    ...
    k.surf <- if (scenario == "I1" && surf == "Elevator") 0 else param$k.surf
    ...
    {(Conc.recover/Conc.seed)*Conc.surf.i/ LR.Film
...
#Plot drawing 
plot_itv <- ggplot(risk_df_comp, aes(x = Scenario, y = Risk, fill = Scenario)) +
  geom_violin(trim = FALSE, alpha = 0.6) +
  geom_boxplot(width = 0.1, outlier.size = 0.5, alpha = 0.8) +
  ...
  scale_y_continuous(trans = "log10", 
                     labels = scales::scientific,
                     breaks = c(1e-22, 1e-20, 1e-18, 1e-16, 1e-14, 1e-12, 1e-10, 1e-8, 1e-6),
                     limits = c(1e-22, max(risk_df_comp$Risk))) +
  labs(y = "Risk (log10)", x = "Scenario") +
  theme_bw()
...
run_touch_sequence_SA2 <- function(sequence, scenario, iter, intervention = NULL) {
  ...
  for (i in seq_along(sequence)) {
    surf <- sequence[i]
    param <- get_surface_params(surf, iter)
    k.surf <- if (surf == "Elevator") 0 else param$k.surf
    Conc.recover <- param$Conc.recover
    
    
    #Concentration on each fomite after 4 hours 
    
    Conc.4hour<-if (surf == "Elevator")(Conc.recover/Conc.seed)*Conc.surf.i/LR.Film
    else (Conc.recover/Conc.seed)*Conc.surf.i
...
spear.m2_E<-data.frame(risk_vec, Conc.h.inf, Conc.h.inf_gc, Conc.seed, Conc.recover, Frac.hf, Frac.hs, gc_PFU, hand_glove, k_gl, k.hand, k.surf, k.surf.cu,
                       LR.Film, LR.HS, LR.S, RE.rinse, T.handarea, TE.fil_h, TE.gf, TE.h_fil, TE.hf,TE.hs, TE.sh)
