...
run_touch_sequence_2 <- function(sequence, scenario, iter, intervention = NULL) {
  ...
  k.surf <- if (scenario %in% c("I1", "I1+I2", "I1+I2+I3") && surf1 == "Elevator") 0 else param1$k.surf
  ...
  for (i in seq_along(sequence)) {
    ...
    k.surf <- if (scenario %in% c("I1", "I1+I2", "I1+I2+I3") && surf == "Elevator") 0 else param$k.surf
    Conc.recover <- param$Conc.recover
    
    
    #Concentration on each fomite after 4 hours 
    
    Conc.4hour<-if (scenario %in% c("I1", "I1+I2", "I1+I2+I3")&& surf == "Elevator"){(Conc.recover/Conc.seed)*Conc.surf.i/LR.Film}
