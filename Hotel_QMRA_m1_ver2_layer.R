...
run_touch_sequence <- function(sequence, scenario, iter, intervention = NULL) {
  ...
  TE.sh1 <- if (scenario %in% c("I1", "I1+I2", "I1+I2+I3")) TE.h_fil 
  else param1$TE.sh
  
  TE.hs1 <- if (scenario %in% c("I1", "I1+I2", "I1+I2+I3")) TE.fil_h 
  else param1$TE.hs
...
ggsave("Infection risk_m1_layer.tiff", dpi=600, dev= 'tiff', height=6, width=20, units='in')
...
ggsave("Infection risk_m1_layer (Elevator).tiff", dpi=600, dev= 'tiff', height=6, width=9, units='in')
